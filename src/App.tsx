import { Key, useCallback, useEffect, useReducer, useState } from 'react'
...
import { generateBillboard, generateRectangle, generateEllipse, generatePointer, generateAnimatedBillboard, getStartTime, getStopTime, FIRE_RED, getRandomNumber, RESOURCE_BUBBLES, getRandomBubble } from "./Utils";
...
function App() {
    ...
    function goToResource(resource: any) {
        viewer.flyTo(
            viewer.entities.getById(`pointer_${resource.name}`) as Entity,
            { 
                duration: 1.5, 
                offset: new HeadingPitchRange(CesiumMath.toRadians(resource.brng), 
                CesiumMath.toRadians(-40), 20) 
            }
        );
    }
